...
class Roomba:
    ...
    def periodic_connection(self) -> None:
        ...
        while not self.stop_connection:
            try:
                ...
                self.log.warning(f"Periodic connection lost due to {error}")
                return
    ...
    def on_message(
        ...
        if decoded_message is None:
            self.log.warning(f"Got malformed message from {client_ip}: {msg}")
            return

        self.dict_merge(self.master_state, decoded_message)
        self.log.debug(f"Received message from {client_ip}: {msg}")
        self.decode_topics(decoded_message)

        # default every 5 minutes
        if time.time() - self.time > self.update_seconds:
            self.log.debug(f"Publishing master_state {client_ip}")
    ...
    def set_preference(self, name: str, value: RobotPreference) -> None:
        self.log.debug("Set preference: %s, %s", name, value)
        val = value
    ...
    def decode_topics(
        self, state: RoombaMessage, prefix: Optional[str] = None
    ) -> None:
        """
...
def _decode_payload(raw_payload: bytes) -> Optional[RoombaMessage]:
    try:
